...
1.get_indexという関数でIT系のインデックスページをクローリング → URLのスクレイピングを行い、    
2.get_detailという関数で詳細ページをクローリング → 必要な文字列のスクレイピングを行っています。  
...
のように品詞ごとにスペース区切ることを分かち書きと言います。  
MeCabという形態素解析器を使って簡単に行えます。  
MeCabについては[こちら](https://github.com/tomboy-jp/MeCab_dep)からどうぞ。
...
L1正規化を行うと特徴の数が減ってしまうため、全体像を眺めたい派としては消化不良になってしまいます。  
とはいえあまり意味のなさそうな単語もちらほら混じっているのでこれらをストップワードに追加するのは精度改善に貢献しそうですね。  
